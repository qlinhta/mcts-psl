...
def NRPA(level, node, strategy, log_file, iterations, alpha, start_time, time_horizon):
    if time.time() - start_time > time_horizon:
        return Grid()
    ...
    if level == 0:
        ...
        for _ in range(iterations):
            if time.time() - start_time > time_horizon:
                break
            result = NRPA(level - 1, node.copy(), strategy, log_file, iterations, alpha, start_time, time_horizon)
